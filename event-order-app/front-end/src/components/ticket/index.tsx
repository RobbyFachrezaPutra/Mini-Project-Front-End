...
export default function TicketDialog({
  ...
  const TicketSchema = Yup.object().shape({
    ...
    quota: Yup.number()
      ...
      .test(
        "is-less-or-equal-seats",
        `Quota can't be more than remaining seats (${seats})`,
        (value) => {
    ...
    sales_start: Yup.date()
      ...
      .test(
        ...
        function (value) {
          const sales_end = this.parent.sales_end;
          if (!value || !sales_end) return true; // skip kalau salah satu belum ada
          return value <= sales_end;
        }
      )
      .test(
        "is-not-after-event-end",
        "Start sales date can't be after event end date",
        function (value) {
          if (!value || !end_date) return true;
          const eventEndDate = new Date(end_date);
          return value <= eventEndDate;
        }
      ),
    sales_end: Yup.date()
      ...
      .test(
        "is-after-start",
        "Sales End date must be after Sales start",
        function (value) {
          const sales_start = this.parent.sales_start;
          if (!value || !sales_start) return true;
          return value >= sales_start;
        }
      )
      .test(
        "is-not-after-event-end",
        "Sales end can't be after event end",
        function (value) {
          if (!value || !end_date) return true;
          const eventEndDate = new Date(end_date);
          return value <= eventEndDate;
              ...
              <Form className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <DatePicker
                      selected={
                        values.sales_start ? new Date(values.sales_start) : null
                      }
                  ...
                  <div>
                    <DatePicker
                      selected={
                        values.sales_end ? new Date(values.sales_end) : null
                      }
                      onChange={(date: Date | null) => {
