...
export default function BuyTicketDialog({ open, onClose, eventId }: BuyTicketDialogProps) {
  const [tickets, setAvailableTickets] = useState<ITicketParam[]>([]);
  const [selectedQuantities, setSelectedQuantities] = useState<{ [ticketId: number]: number }>({});
  const [vouchers, setAvailableVouchers] = useState<IVoucherParam[]>([]);
  const [selectedVoucher, setSelectedVoucher] = useState<IVoucherParam | null>(null);
  ...
  useEffect(() => {
    if (open) {
      ...
      const fetchTickets = async () => {
        try {
          const res = await api.get(`${process.env.NEXT_PUBLIC_API_URL}/api/eventorder/tickets/by-event/${eventId}`);
      ...
      const fetchVouchers = async () => {
        try {
          const res = await api.get(`${process.env.NEXT_PUBLIC_API_URL}/api/eventorder/vouchers/by-event/${eventId}`);
      ...
      const fetchCoupon = async () => {
        try {
          const res = await api.get(`${process.env.NEXT_PUBLIC_API_URL}/api/eventorder/coupons/by-user/${storedUser.id}`);
      ...
      const fetchPoint = async () => {
        try {
          const res = await api.get(`${process.env.NEXT_PUBLIC_API_URL}/api/eventorder/points/by-user/${storedUser.id}`);
  ...
  const subtotal = Object.entries(selectedQuantities).reduce((acc, [ticketId, qty]) => {
    const ticket = tickets.find((t) => t.id === parseInt(ticketId));
    return acc + (ticket ? ticket.price * qty : 0);
  }, 0);
  
  const discount = selectedVoucher?.discount_amount || 0;
  const discountCoupon = coupon && useCoupon
  ? subtotal * (coupon[0].discount_amount / 100)
  : 0;

  const totalPrice = Math.max(0, subtotal - discount - discountCoupon - pointUsed); // jaga-jaga biar gak negatif
  ...
  const handleSubmit = async () => {
    try {
      ...
      const res = await api.post(`${process.env.NEXT_PUBLIC_API_URL}/api/eventorder/transactions`, payload);
    ...
    <Dialog open={open} onClose={onClose} className="fixed z-10 inset-0 overflow-y-auto">
      <div className="flex items-center justify-center min-h-screen px-4">
        <Dialog.Panel className="bg-white rounded-lg shadow-xl w-full max-w-2xl p-6">
          <Dialog.Title className="text-xl font-bold mb-6">Pilih Tiket</Dialog.Title>
          <div className="flex flex-cols-2 gap-4">
            <div>
            {tickets.map((ticket) => (
              <div key={ticket.id} className="w-[300px] border p-4 rounded-md mb-4">
                <div className="flex justify-between">
                  <div>
                    <h2 className="font-semibold text-lg">{ticket.name}</h2>
                    <p className="text-gray-600">{ticket.description}</p>
                    <p className="text-sm text-gray-500">Remaining Seats: {ticket.remaining}</p>
                    <p className="text-sm text-gray-700">Price: Rp {ticket.price.toLocaleString()}</p>
                  </div>
                  <div>
                    <select
                    ...
                    >
                      {[0, 1, 2, 3, 4, 5].map((qty) => (
                        <option key={qty} value={qty} disabled={qty > ticket.remaining}>
                          {qty}
                        </option>
                      ))}
                    </select>
          ...
          <p className="text-lg font-semibold">Discount Voucher: Rp {selectedVoucher?.discount_amount.toLocaleString()}</p>
          <p className="text-lg font-semibold">Discount Point : {pointUsed.toLocaleString()}%</p>
          <p className="text-lg font-semibold">Discount Coupon : {coupon[0]?.discount_amount.toLocaleString()}%</p>
            ...
            <div>
              {vouchers.map((voucher) => (
                ...
                className={`w-[300px] border p-4 rounded-md mb-4 cursor-pointer ${
                  selectedVoucherId === voucher.id ? "bg-blue-100 border-blue-500" : ""
                }`}
                >
                  <div className="flex justify-between">
                    <div>
                    {voucher.code}
                      <h2 className="font-semibold text-lg">{voucher.code}</h2>
                      <p className="text-gray-600">{voucher.description}</p>
                      <p className="text-sm text-gray-700">Discount Amount: Rp {voucher.discount_amount.toLocaleString()}</p>
                    </div>
                  </div>
                </div>
              ))}
                ...
                <div className="mb-4">
                  <label className="flex items-center">
                    <input
                      ...
                      className="mr-2"
                    />
                    <span>{coupon[0]?.code} ({coupon[0]?.discount_amount}% diskon)</span>
                  </label>
                </div>
              )} 
              <div className="mb-4">
                <label className="block font-medium mb-1">Poin (Max: {userPoint[0]?.point ? userPoint[0]?.point : 0})</label>
                <input
                  type="number"
