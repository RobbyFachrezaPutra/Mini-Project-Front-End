...
const EventSchema = Yup.object().shape({
  ...
  status: Yup.string()
    ...
    .test("is-valid-status", "Status must be selected from combobox", function (value) {
...
export default function EventForm() {
  ...
  useEffect(() => {
    const user = JSON.parse(localStorage.getItem("user") || "null") as IUserParam
    setStoredUser(user);
    api
      ...
      <Formik<IEvent>
        ...
        onSubmit={async (values) => {
          try {
            ...
            formData.append("description", JSON.stringify(editor?.getJSON() || {}));
            ...
            formData.append("available_seats", values.available_seats.toString());
      ...
      >
        {({ values, setFieldValue, setFieldTouched }) => (
          <Form className="space-y-4">
            ...
            <div className="mb-4">
              <Field as="select" name="category_id" className="py-2 px-3 rounded-xl text-black border-2 border-stone-50 bg-white w-full sm:w-auto">
                ...
                <div className="mt-4">
                  {values.tickets.map((ticket: any, index: number) => (
                    <div key={index} className="p-2 border rounded mb-2">
                      ...
                      <p className="font-semibold">🎟 {ticket.name} ({ticket.type})</p>
                      <p>{ticket.description}</p>
                      <p>Qty: {ticket.quota} | Price: {ticket.price}</p>
                      <p>Sell: {ticket.sales_start.toLocaleDateString()} - {ticket.sales_end.toLocaleDateString()}</p>
                      <button
                        type="button"
                ...
                <div className="mt-4">
                  {values.vouchers.map((voucher: any, index: number) => (
                    <div key={index} className="p-2 border rounded mb-2">
                      ...
                      <p>Sell: {voucher.sales_start.toLocaleDateString()} - {voucher.sales_end.toLocaleDateString()}</p>
                      <button
                        type="button"
                        onClick={() => remove(index)}
                        className="mt-1 px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-md"
            ...
            <TicketDialog
              ...
              onAddTicket={(ticket) => setFieldValue("tickets", [...values.tickets, ticket])}
            />
            <VoucherDialog
              open={voucherDialogOpen}
              onClose={() => setVoucherDialogOpen(false)}
              onAddVoucher={(voucher) => setFieldValue("vouchers", [...values.vouchers, voucher])}
            />            
          </Form>
        )}
      </Formik>
